...
import pakAmarImage from 'figma:asset/e4b5f95419361dfc953fd1b6c7aa9a79f9c4b944.png';
import anggiImage from 'figma:asset/9aa9671e0df4a3ea9a79bf08ee20b747c464fa7b.png';
            ...
            >
              <Card className="border-2 border-[#AE8737] shadow-lg overflow-hidden bg-[#1a1a1a] group h-full">
                ...
                <div className="p-6 text-center bg-[#1a1a1a] border-t-2 border-[#AE8737]">
                  <h3 className="text-2xl font-bold text-white mb-2">{lawyer.name}</h3>
                  <p className="text-[#AE8737] font-semibold text-base mb-1">{lawyer.title}</p>
            ...
            >
              ...
              <div className="flex flex-col md:flex-row w-full">
                ...
                <div className="w-full md:w-7/12 lg:w-8/12 p-8 md:p-12 overflow-y-auto">
                  {/* Header Info */}
                  <div className="mb-10">
                    <h2 className="text-4xl md:text-5xl font-bold text-[#191919] mb-3">{selectedLawyer.name}</h2>
                    <p className="text-2xl text-[#AE8737] font-bold mb-6">{selectedLawyer.title}</p>
                    ...
                    <div className="flex flex-wrap gap-4">
                      <a href={`mailto:${selectedLawyer.email}`} className="flex items-center gap-3 bg-gray-50 px-5 py-3 rounded-full border border-gray-200 hover:border-[#AE8737] transition-colors group">
                        ...
                        <span className="text-[#191919] font-medium">{selectedLawyer.email}</span>
                      </a>
                      <a href={`tel:${selectedLawyer.phone}`} className="flex items-center gap-3 bg-gray-50 px-5 py-3 rounded-full border border-gray-200 hover:border-[#AE8737] transition-colors group">
                        <div className="w-8 h-8 rounded-full bg-[#AE8737] flex items-center justify-center flex-shrink-0 group-hover:bg-[#191919] transition-colors">
                          <Phone className="w-4 h-4 text-white" />
                        </div>
                        <span className="text-[#191919] font-medium">{selectedLawyer.phone}</span>
                      </a>
                      <a href={selectedLawyer.linkedin} className="flex items-center gap-3 bg-gray-50 px-5 py-3 rounded-full border border-gray-200 hover:border-[#AE8737] transition-colors group">
                        <div className="w-8 h-8 rounded-full bg-[#AE8737] flex items-center justify-center flex-shrink-0 group-hover:bg-[#191919] transition-colors">
                          <Linkedin className="w-4 h-4 text-white" />
                        </div>
                        <span className="text-[#191919] font-medium">LinkedIn</span>
                      </a>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-8">
                        {selectedLawyer.experience.map((exp, idx) => (
                          <div key={idx}>
                            <h4 className="font-bold text-[#AE8737] text-lg mb-2">{exp.title}</h4>
                    ...
                    <div>
                      ...
                      <div className="space-y-8">
                        {selectedLawyer.education.map((edu, idx) => (
                          <div key={idx} className="bg-gray-50 p-5 rounded-lg border-l-4 border-[#AE8737]">
                            <div className="flex justify-between items-start mb-1">
                              <h4 className="font-bold text-[#191919] text-lg leading-tight">{edu.degree}</h4>
                              <span className="text-[#AE8737] font-bold text-sm bg-[#AE8737]/10 px-2 py-1 rounded">{edu.year}</span>
                            </div>
                            <p className="text-gray-700 font-medium">{edu.school}</p>
                            <p className="text-sm text-[#AE8737] mb-3 italic">{edu.honor}</p>
                            <ul className="space-y-1">
                              {edu.details.map((detail, dIdx) => (
                                <li key={dIdx} className="text-gray-600 text-xs flex items-start gap-2">
                                  <span>•</span>
                                  <span>{detail}</span>
                                </li>
                              ))}
